...
# 【终极补丁：在所有库被导入前，强行劫持系统的获取终端大小方法】
# 告诉所有尝试获取终端大小的库（如 chat-downloader），虚拟终端大小是 80x24
try:
    os.get_terminal_size = lambda fallback=(80, 24): os.terminal_size((80, 24))
except Exception:
    pass
...
if st.button("🚀 开始抓取并生成报告"):
    ...
    try:
        ...
        st.error(f"抓取失败，请检查链接是否有误或被设为隐私视频: {str(e)}")
        st.stop()
    finally:
        # 恢复环境
        if hasattr(sys.stdout, 'isatty') and 'original_isatty' in locals():
